...
#ifdef USE_PSP_IRKEYB
 static int irkeyb_dragon_key      = 0;
 static int irkeyb_dragon_pending  = 0;
# endif
...
psp_kbd_enter_irkeyb()
{
  ...
  if (psp_irkeyb != PSP_IRKEYB_EMPTY) {
    ...
    if (dragon_key != -1) {
      irkeyb_dragon_key     = dragon_key;
      irkeyb_dragon_pending = 1;
      dragon_key_event(dragon_key, 1);
...
# endif
  ...
  if (new_Lx > 0) {
    dragon_decode_joystick(KBD_JOY_RIGHT);
  } else 
  if (new_Lx < 0) {
    dragon_decode_joystick(KBD_JOY_LEFT);
  }
  if (new_Ly > 0) {
    dragon_decode_joystick(KBD_JOY_UP);
  } else 
  if (new_Ly < 0) {
    dragon_decode_joystick(KBD_JOY_DOWN);
  }

  /* Analog device has moved */
  if (new_Lx > 0) {
    if (old_Lx <= 0) dragon_decode_key(KBD_JOY_RIGHT, 1);
  } else 
  if (new_Lx < 0) {
    if (old_Lx >= 0) dragon_decode_key(KBD_JOY_LEFT , 1);
  } else {
    dragon_decode_key(KBD_JOY_RIGHT , 0);
    dragon_decode_key(KBD_JOY_LEFT  , 0);
  }

  if (new_Ly > 0) {
    if (old_Ly <= 0) dragon_decode_key(KBD_JOY_DOWN, 1);
  } else 
  if (new_Ly < 0) {
    if (old_Ly >= 0) dragon_decode_key(KBD_JOY_UP , 1);
  } else {
    dragon_decode_key(KBD_JOY_DOWN , 0);
    dragon_decode_key(KBD_JOY_UP   , 0);
  }
