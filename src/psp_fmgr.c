  ...
  while(nfiles<PSP_FMGR_MAX_ENTRY){
    ...
    if ( (format == user_file_format            ) || 
         ((format == FMGR_FORMAT_ZIP          ) && 
          ...
          (user_file_format != FMGR_FORMAT_ROM) &&
          (user_file_format != FMGR_FORMAT_DISK)) ) 
    {
      nfiles++;
    }
  }
...
psp_fmgr_menu(int format)
{
  ...
  if (psp_file_request(user_filename, user_filedir)) {
    ...
    else 
    {
      ...
      {
        if (file_format == FMGR_FORMAT_STATE) /* State */ error = dragon_load_state(user_filename);
        else
        if (file_format == FMGR_FORMAT_ROM) /* ROM */ error = dragon_load_rom(user_filename);
        else
        if (file_format == FMGR_FORMAT_DISK) /* Disk */ error = dragon_load_disk(user_filename);
        else
        if (file_format == FMGR_FORMAT_KBD) /* Kbd */  error = psp_kbd_load_mapping(user_filename);
        else
        if (file_format == FMGR_FORMAT_SET) /* Settings */  error = dragon_load_file_settings(user_filename);
      }
    }
