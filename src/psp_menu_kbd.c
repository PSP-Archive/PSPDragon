...
psp_display_screen_kbd_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_KBD_ITEM; menu_id++) 
  {
        ...
        (menu_id <= MENU_KBD_JOY_RIGHT)) 
    {
      ...
      if (menu_kbd_selected == KBD_NORMAL_MAPPING  ) dragon_key = loc_kbd_mapping[kbd_id];
      else
      if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) dragon_key = loc_kbd_mapping_L[kbd_id];
      else
      if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) dragon_key = loc_kbd_mapping_R[kbd_id];

      if ((dragon_key >= 0) && (dragon_key < DRAGONK_MAX_KEY)) {
        strcpy(buffer, psp_dragon_key_info[dragon_key].name);
      } else 
      if (dragon_key == KBD_UNASSIGNED) {
        sprintf(buffer, "UNASSIGNED");
      } else
      if (dragon_key == KBD_LTRIGGER_MAPPING) {
        sprintf(buffer, "L MAPPING");
      } else
      if (dragon_key == KBD_RTRIGGER_MAPPING) {
        sprintf(buffer, "R MAPPING");
      } else {
        sprintf(buffer, "KEY %d", dragon_key);
      }
...
psp_keyboard_menu_mapping(int kbd_id, int step)
{
  ...
  if (ploc_kbd_mapping[kbd_id] <  -3) ploc_kbd_mapping[kbd_id] = DRAGONK_MAX_KEY-1;
  else
  if (ploc_kbd_mapping[kbd_id] >= DRAGONK_MAX_KEY) ploc_kbd_mapping[kbd_id] = -3;
}
...
# ifdef USE_PSP_IRKEYB
    ...
    if (psp_irkeyb != PSP_IRKEYB_EMPTY) {
      ...
      if (psp_irkeyb == 0x8) {
        dragon_key = DRAGONK_DELETE;
      } else
      if (psp_irkeyb == 0x9) {
        dragon_key = DRAGONK_TAB;
      } else
      if (psp_irkeyb == 0xd) {
        dragon_key = DRAGONK_RETURN;
      } else
      if (psp_irkeyb == 0x1b) {
        dragon_key = DRAGONK_JOY_FIRE;
      } else
      if (psp_irkeyb == PSP_IRKEYB_UP) {
        dragon_key = DRAGONK_UP;
      } else
      if (psp_irkeyb == PSP_IRKEYB_DOWN) {
        dragon_key = DRAGONK_DOWN;
      } else
      if (psp_irkeyb == PSP_IRKEYB_LEFT) {
        dragon_key = DRAGONK_LEFT;
      } else
      if (psp_irkeyb == PSP_IRKEYB_RIGHT) {
        dragon_key = DRAGONK_RIGHT;
      } else {
        dragon_key = dragon_get_key_from_ascii(psp_irkeyb);
      }
      ...
      if (dragon_key != -1) {
            ...
            (cur_menu_id <= MENU_KBD_JOY_RIGHT)) 
        {
          ...
          if (menu_kbd_selected == -1) loc_kbd_mapping[kbd_id] = dragon_key;
          else
          if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) loc_kbd_mapping_L[kbd_id] = dragon_key;
          else
          if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) loc_kbd_mapping_R[kbd_id] = dragon_key;
        }
      }
    }
# endif
