...
loc_dragon_save_settings(char *chFileName)
{
  ...
  if (FileDesc != (FILE *)0 ) {
    ...
    fprintf(FileDesc, "dragon_vsync=%d\n"          , DRAGON.dragon_vsync);
...
loc_dragon_load_settings(char *chFileName)
{
  ...
  while (fgets(Buffer,512, FileDesc) != (char *)0) {
    ...
    *Scan = '\0';
    ...
    if (!strcasecmp(Buffer,"dragon_vsync"))  DRAGON.dragon_vsync = Value;
  }
...
# endif
  ...
  for (index = 0; index < DRAGON_MAX_SAVE_STATE; index++) {
    ...
    memset(&DRAGON.dragon_save_state[index].date, 0, sizeof(ScePspDateTime));
...
# endif
  ...
  if (save_id < DRAGON_MAX_SAVE_STATE) {
    ...
    if (! error) {
      ...
      DRAGON.dragon_save_state[save_id].thumb = 0;
      memset(&DRAGON.dragon_save_state[save_id].date, 0, sizeof(ScePspDateTime));
