...
# define MENU_LOAD_DISK    7
# define MENU_LOAD_SNA     8
# define MENU_EJECT_ROM    9

# define MENU_KEYBOARD    10
# define MENU_SETTINGS    11

# define MENU_HARD_RESET  12
# define MENU_BACK        13
# define MENU_EXIT        14
...
psp_menu_display_save_name()
{
  ...
  snprintf(buffer, 20, "Game: %s", DRAGON.dragon_save_name);
...
psp_display_screen_menu(void)
{
  ...
  //if (psp_menu_dirty) 
  {
    ...
    psp_sdl_back2_print( 330, 6, " R: Soft Reset ", PSP_MENU_NOTE_COLOR);
  ...
  for (slot_id = 0; slot_id < DRAGON_MAX_SAVE_STATE; slot_id++) {
    ...
    if (DRAGON.dragon_save_state[slot_id].used) {
      ScePspDateTime* my_date = &DRAGON.dragon_save_state[slot_id].date;
# ifndef LINUX_MODE
...
# endif
  ...
  if (DRAGON.dragon_save_state[cur_slot].thumb) {
    psp_sdl_blit_thumb(320, 40, DRAGON.dragon_save_state[cur_slot].surface);
  } else {
...
psp_main_menu_soft_reset()
{
  ...
  psp_sdl_back2_print(140, 160, "Soft Reset Dragon !", 
...
psp_main_menu_hard_reset()
{
  /* Reset ! */
  psp_display_screen_menu();
  psp_sdl_back2_print(140, 160, "Hard Reset Dragon !", 
                     PSP_MENU_WARNING_COLOR);
  psp_menu_dirty = 1;
  psp_sdl_flip();
