...
psp_sdl_save_thumb_png(SDL_Surface* my_surface, char* filename)
{
  ...
  src_pixel += DRAGON_WIDTH * 24;
  for (y = 0; y < REAL_DRAGON_H; y += 2) {
    for (x = 0; x < REAL_DRAGON_W; x += 2) {
      *dst_pixel++ = src_pixel[32 + x];
    }
    src_pixel += DRAGON_WIDTH * 2;
  }
...
psp_sdl_save_screenshot(void)
{
  ...
  sprintf(TmpFileName,"%s/scr/screenshot_%d.png", DRAGON.dragon_home_dir, DRAGON.psp_screenshot_id++);
  if (DRAGON.psp_screenshot_id >= 10) DRAGON.psp_screenshot_id = 0;
...
# endif
  ...
  for (Index = 0; Index < DRAGON_MAX_SAVE_STATE; Index++) {
    DRAGON.dragon_save_state[Index].surface = 
       SDL_CreateRGBSurface(SDL_SWSURFACE, 
                            SNAP_WIDTH, SNAP_HEIGHT,
                            blit_surface->format->BitsPerPixel,
                            blit_surface->format->Rmask,
                            blit_surface->format->Gmask,
                            blit_surface->format->Bmask, 0);
  }
